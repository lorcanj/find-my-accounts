...
describe('mboxImportService', () => {
  ...
  describe('slice + FileReader fallback (older browsers)', () => {
    let mockFileReader;

    beforeEach(() => {
      // File does NOT have stream() method
      delete mockFile.stream;

      // Mock FileReader - the service will assign onload and onerror handlers
      mockFileReader = {
        readAsArrayBuffer: vi.fn(),
        onload: null,  // Will be set by the service
        onerror: null, // Will be set by the service
        result: null
      };
    ...
    it('should use slice + FileReader when stream is not available', async () => {
      const promise = importMboxFile(mockFile, null, null);
      ...
      // Verify end message sent (file size = 1000, so after first chunk we're done)
      expect(mockWorker.postMessage).toHaveBeenCalledWith({ type: 'end' });
    ...
    it('should read file in 5MB chunks', async () => {
      ...
      // First chunk should be 0 to 5MB
      expect(mockFile.slice).toHaveBeenCalledWith(0, CHUNK_SIZE);

      // Simulate first read completing
      mockFileReader.result = new ArrayBuffer(CHUNK_SIZE);
      mockFileReader.onload({ target: { result: mockFileReader.result } });
      await new Promise(resolve => setTimeout(resolve, 0));
    ...
    it('should handle partial final chunk correctly', async () => {
      // File size not evenly divisible by chunk size
    ...
    it('should reject and terminate worker on FileReader error', async () => {
      const promise = importMboxFile(mockFile, null, null);
      await new Promise(resolve => setTimeout(resolve, 0));
